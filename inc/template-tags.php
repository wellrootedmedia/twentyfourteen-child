...
function twentyfourteen_alternate_post_thumbnail($postId) {
    $format = get_post_format( $postId );
    $youTubeValue = get_post_meta( $postId, 'YouTubeId' );

    if ( is_singular() ) {

        if($format != "video") {
            ?>
            <img width="672" height="372"
                 class="attachment-post-thumbnail wp-post-image"
                 src="data:image/png;base64,"
                 data-src="<?php echo get_stylesheet_directory_uri(); ?>/js/holder.js/672x372/auto/#777:#7c7c7c/text:<?php the_title(); ?> placement image"
                 alt="<?php the_title(); ?>">
            <?php
        }
        ...
        if($format == "video" && $youTubeValue) {
            $thumb = getYouTubeThumbnail($youTubeValue[0]);
            ?>
            <a class="post-thumbnail" href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
                <img width="672" height="372" class="attachment-post-thumbnail wp-post-image" src="<?php echo $thumb; ?>" />
            </a>
        <?php
        }
